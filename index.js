...
`}function R(){n.info({message:"We're sorry, but you've reached the end of search results.",position:"topRight"})}const a={usrSearch:document.querySelector("input"),usrForm:document.querySelector("form"),showMore:document.querySelector(".btn-load")};let d,s,h;a.usrForm.addEventListener("submit",async e=>{if(e.preventDefault(),d=a.usrSearch.value,!!a.usrSearch.value.trim()){$(),w();try{s=1,b();const r=await p(d,s);if(h=Math.ceil(r.totalHits/m),u(),r.hits.length===0){n.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"}),a.usrSearch.value="";return}P(r.hits),a.usrSearch.value="",h>1?S():R()}catch{u(),a.usrSearch.value="",n.error({message:"Something went wrong!",position:"topRight"})}}});a.showMore.addEventListener("click",async e=>{e.preventDefault(),s+=1,w(),b();try{const r=await p(d,s);E(r.hits);const i=l.firstElementChild.getBoundingClientRect();window.scrollBy({top:i.height*2,behavior:"smooth"}),s>=h?R():S()}catch{n.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"})}finally{u()}});
